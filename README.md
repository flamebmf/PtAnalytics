...
Файлы `.example` содержат шаблоны без реальных паролей/IP и отслеживаются в git.
Локальные `.yaml` исключены через `.gitignore`.
...
- Скрыты пагинация и колонки класс/камера
...
Текущая модель: 3 класса (mazda, lena, im), mAP50=0.885.
...
```
cam/
├── config/                # конфиги (.example — шаблоны для git)
│   ├── cameras.yaml       # RTSP-камеры (локальный, в gitignore)
│   ├── settings.yaml      # глобальные настройки (локальный)
│   ├── triggers.yaml      # триггеры (локальный)
